terraform {
  backend "s3" {                                    # Terraform 상태 파일(terraform.tfstate)을 로컬이 아닌 S3에 저장하도록 지정
    bucket       = "courseitda-dev-terraform-state" # 상태 파일을 저장할 S3 버킷 이름
    key          = "dev/terraform.tfstate"          # S3 버킷 안에서 사용할 객체 키(= 경로) ex. courseitda-terraform-state-dev/dev/terraform.tfstate
